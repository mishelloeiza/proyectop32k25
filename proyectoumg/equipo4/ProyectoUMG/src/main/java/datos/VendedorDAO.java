...
public class VendedorDAO {
    ...
    public List<Vendedor> select() {
        ...
        try {
            conn = Conexion.getConnection();//obtiene informacion de la base de datos
            stmt = conn.prepareStatement(SQL_SELECT);
            rs = stmt.executeQuery();
            while (rs.next()) {//Mientras exista un siguiente(exista un registro)
                int id_vendedor = rs.getInt("id_vendedor");
                String nombre = rs.getString("nombrevendedor");
                String direccion = rs.getString("direvendedor");
                
                vendedor = new Vendedor(); //vendedor(Objeto) Vendedor(clase)
